...
import {chatWithAuditApp, chatWithAuditAppWithFile} from "@/api/index.js";
...
const generateChatId = () => {
  return 'audit_' + Math.random().toString(36).substring(2, 10)
}
...
const sendMessage = (messageData) => {
  ...
  // 如果有文件，使用带文件的聊天函数
  if (file) {
    eventSource = chatWithAuditAppWithFile(message, chatId.value, file)
  } else {
    eventSource = chatWithAuditApp(message, chatId.value)
  }
...
onMounted(() => {
  ...
  // 添加欢迎消息
  addMessage('你好，我有丰富的审计专业知识储备，请告诉我你的审计问题，我会尽力给予帮助和建议。', false)
})
